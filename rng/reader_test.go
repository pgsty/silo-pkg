...
func TestResetSizeProducesUniqueOutput(t *testing.T) {
	// One aligned block is all the assertion needs: ResetSize reseeds the
	// subkeys and nothing else, so if they stayed zero the very first bytes
	// after the reset would repeat.
	const size = 1024
	...
	if err := r.ResetSize(size); err != nil {
		t.Fatal(err)
	}
	out2 := make([]byte, size)
